...
def main(file):
    ...
    #break beta and gamma into chunks of size window
    #evaluate correlation between beta and gamma for each chunk
    #plot correlation vs time

    beta_pieces = np.array_split(beta, 1)
    gamma_pieces = np.array_split(gamma, len(gamma)//window)

    corr = []
    for i in range(len(beta)):
        corr.append(np.corrcoef(beta[i], gamma[i])[0, 1])

    plt.figure(figsize=(10, 6))
    plt.plot(corr, label='Correlation')
    #plot rolling mean of correlation
    plt.plot(pd.Series(corr).rolling(window=3).mean(), label='3-rolling mean of correlation')
    plt.xlabel('Days')
    plt.ylabel('Correlation')
    plt.title('Correlation between Beta and Gamma')
    plt.legend()
    plt.show()
