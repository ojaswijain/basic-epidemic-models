...
def cvxopt_beta(beta, S, I):
    """
    @params: beta, S, I
    @returns: beta based on convex optimization
    """
    b = cp.Variable(1)
    objective = cp.Minimize(cp.sum(cp.abs(beta-b)*S*I))
    constraints = [b >= 0]
...
def cvxopt_gamma(gamma, I):
    """
    @params: gamma, I
    @returns: gamma based on convex optimization
    """
    g = cp.Variable(1)
    objective = cp.Minimize(cp.sum(cp.abs(gamma-g)*I))
    constraints = [g >= 0]
    prob = cp.Problem(objective, constraints)
    result = prob.solve()
    return g.value
...
def main():
    ...
    beta_opt = cvxopt_beta(beta, S_cum, I_cum)
    gamma_opt = cvxopt_gamma(gamma, I_cum)
