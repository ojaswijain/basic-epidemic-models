...
def extrapolate_beta_gamma(beta_weekly, gamma_weekly):
    """
    @params: beta, gamma, forecast
    @returns: beta, gamma
    Spline interpolation on last history values of beta and gamma
    Use extrapolation to predict beta and gamma for next forecast values 
    """
    history = len(beta_weekly)
    beta = np.zeros(history+forecast)
    gamma = np.zeros(history+forecast)
    #generate spline for beta and gamma
    beta_spline = CubicSpline(np.arange(history), beta_weekly[-history:])
    gamma_spline = CubicSpline(np.arange(history), gamma_weekly[-history:])
    #predict beta and gamma for next forecast values
    beta[history-1:] = beta_spline(np.arange(history-1, history+forecast))
    gamma[history-1:] = gamma_spline(np.arange(history-1, history+forecast))
...
def main():
    ...
    #Store weekwise beta and gamma
    beta_compressed = np.array_split(beta_opt, len(beta_opt)/window)
    gamma_compressed = np.array_split(gamma_opt, len(gamma_opt)/window)
    beta_compressed = np.array([np.median(x) for x in beta_compressed])
    gamma_compressed = np.array([np.median(x) for x in gamma_compressed])

    #Extrapolate beta and gamma
    pred_beta, pred_gamma = extrapolate_beta_gamma(beta_compressed[:25], gamma_compressed[:25])
    #plot
    plt.figure(figsize=(10, 6))
    plt.plot(beta_compressed[:28], label='Beta')
    plt.plot(gamma_compressed[:28], label='Gamma')
    plt.plot(np.arange(24, 28), pred_beta[24:], label='Predicted Beta')
    plt.plot(np.arange(24, 28), pred_gamma[24:], label='Predicted Gamma')
    plt.legend()
    #x and y labels
    plt.xlabel('Week')
    plt.ylabel('Value')
